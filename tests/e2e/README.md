...
- Board/post persistence, editing, votes, task assignment/status, deletion, two-session delivery, a separate board, owner-only access, membership revocation, database stop/restart, notification loss, and recovery of changes saved during a stream disconnect without clearing an open draft. SQL checks distinguish failed writes from saved data whose notification failed.
