...
PR review and the first Linux CI run found a runner ownership defect: root wrote private fault requests that the host user could not read. The image now uses the host UID/GID. A Linux check reproduced the old EACCES failure and passed the real fault exchange with matching IDs. The Docker build now copies explicit source inputs and excludes local credentials and databases; root and nested sentinel files were absent from the built image. Workflow actions use verified immutable release tags where available and commit SHAs otherwise; package install scripts are disabled. `setup-node@v7.0.0` and `setup-docker-action@v5.5.0` have immutable releases and resolve to the same tested commits. Checkout remains pinned by SHA because its `v7.0.1` release is not immutable. Major tags such as `v7` can still move. Runtime and security reviewers approved these fixes. Run `run-473b61eb-7c84-411e-b310-c95e05b16320` passed all three isolated journeys in 4.1 minutes. Lint, TypeScript, all 348 Jest tests, and seven runner checks passed after the callback review fixes. Linux CI passed on commit `67d48cc` after the Corepack change (run `35807983185`).

The next Linux CI run passed the real email/session and guest journeys. It exposed a board-test synchronization gap: after reload, the deleted-card absence check could pass before React hydration, and the member-dialog click could be lost. All three owner reloads now wait for the existing realtime Connected state before interaction. The revocation response listener starts immediately before the removal confirmation. Independent runtime review approved this test-only correction. Fresh local run `run-ebb37cda-3167-462e-855b-f539164c30d4` and Linux CI run `35773066089` passed all three journeys.

Scanner triage preserved all rules and quality limits. Four Sonar findings were marked false positive: an inert HOME string in a pure environment test and three deliberate external-network probes with no credentials. Five Codacy findings were marked false positive after independent security review: two readiness URLs constructed from fixed loopback origins and a bounded numeric slot, and three HTML alerts on fixed React output used only in Jest string assertions. Real image, action-pinning, install-script, and callback findings were fixed in code.

The remaining 21 open Sonar maintainability findings were addressed in code: smaller setup and relay functions, native status elements, read-only props, simpler configuration selection, and a JWT fingerprint assertion that cannot print tokens. The entrypoints use top-level await with a static module import to prevent the dynamic import cycle from blocking startup. Three independent security, database, and runtime reviewers approved the diff. Lint, TypeScript, all 348 Jest tests, and seven runner checks passed. Fresh isolated run `run-8ac4eb92-dcc3-4549-8c5f-2983883245d4` passed all three journeys in 4.2 minutes and cleaned up successfully. Read the current PR checks for cloud scanner results. Codacy had no open findings before this update. No scanner rules or quality limits were changed.
