...
PR review and the first Linux CI run found a runner ownership defect: root wrote private fault requests that the host user could not read. The image now uses the host UID/GID. A Linux check reproduced the old EACCES failure and passed the real fault exchange with matching IDs. The Docker build now copies explicit source inputs and excludes local credentials and databases; root and nested sentinel files were absent from the built image. Workflow actions are pinned to commit SHAs and package install scripts are disabled. Runtime and security reviewers approved these fixes. Run `run-473b61eb-7c84-411e-b310-c95e05b16320` passed all three isolated journeys in 4.1 minutes. Lint, TypeScript, all 348 Jest tests, and seven runner checks passed after the callback review fixes. Updated GitHub CI remains pending.
