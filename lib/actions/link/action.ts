...
export async function processMagicLinkAction(token: string): Promise<never> {
  ...
  if (!(await isAuthenticated())) {
    ...
    redirect(`/api/auth/login?post_login_redirect_url=${encodeURIComponent(`/invite/${token}`)}`);
  }
 ...
 */
export async function validateMagicLinkAction(token: string): Promise<MagicLinkUsageResult> {
  try {
    // Find the magic link
    const link = await findValidLinkByToken(token);
    if (!link) {
    ...
    if (!(await isAuthenticated())) {
      return {
        ...
        redirectUrl: `/api/auth/login?post_login_redirect_url=${encodeURIComponent(`/invite/${token}`)}`
      };
    }
    ...
    return {
      success: true,
      boardId: link.boardId,
      message: `You will be added to "${link.boardTitle}" as a ${link.role === 1 ? 'member' : 'guest'}.`
    };
  } catch (error) {
 ...
 */
export async function getMagicLinkInfoAction(token: string) {
  try {
    const link = await findValidLinkByToken(token);
    if (!link) {
      return null;
